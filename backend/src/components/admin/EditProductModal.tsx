...
import { Loader2, PackagePlus, DollarSign, Archive, Type, Image as ImageIcon, Tag, Hash, FileText, Plus, X, UploadCloud, Link as LinkIcon, PenSquare } from "lucide-react";
...
export default function EditProductModal({ isOpen, onClose, product }: EditProductModalProps) {
    ...
    const initialCategoryFull = product?.category || "Men";
    const [baseCat, ...genderParts] = initialCategoryFull.split(' - ');
    const initialBaseCat = ["Men", "Women", "Footwear", "Accessories"].includes(baseCat) ? baseCat : "Men";
    const initialGender = genderParts.length > 0 ? genderParts.join(' - ') : "All";
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            {/* Modal Content */}
            <div className="relative w-full max-w-4xl h-[90vh] mx-4 rounded-[2rem] overflow-hidden bg-[#0a0a0a]/95 border border-white/10 backdrop-blur-3xl shadow-2xl flex flex-col ring-1 ring-white/5 transform transition-all">
                ...
                {/* Header */}
                <div className="sticky top-0 z-20 flex items-center justify-between px-8 py-6 border-b border-white/5 bg-[#0a0a0a]/80 backdrop-blur-xl">
                    <div>
                        <h2 className="text-xl font-semibold tracking-tight text-white mb-0.5">Edit Product</h2>
                        <p className="text-xs text-gray-400 font-medium">Currently modifying <span className="text-white">{product.title}</span>.</p>
                    </div>
                    <button
                        onClick={onClose}
                        disabled={loading}
                ...
                {/* Form Body */}
                <div className="p-8 relative z-10 w-full overflow-y-auto">
                    {error && (
